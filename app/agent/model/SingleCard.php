...
class SingleCard extends Card {
	...
	public function add($add) {
		...
		$this->log->menber('3',"生成单卡:{$card_add['name']}, 数量:{$add['count']}, 成功!",'2');
	...
	public function delete($id) {
		...
		$this->con->delete('single_card',"id in ({$id}) and menber_id=" . Auth::get('id'));
		$this->log->menber('3','删除单卡(个数):' . (substr_count($id,',') + 1) .', 成功!','2');
