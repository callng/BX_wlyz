...
<div  class="main container-fluid py-3">
  ...
  <li class="nav-item">
    <a class="nav-link" id="user_log-tab"  data-toggle="tab" href="#user_log" role="tab" aria-controls="user_log">用户日志</a>
  </li>
</ul>
...
<div class="form-inline">
    ...
    <div class="form-group">   
       归属管理：
    <input type="text" class="form-control" id="search_user_menber_name" style="width: 150px;" >&nbsp;
    </div>
...
<script type="text/javascript">
  ...
  <div class="input-group mb-2 mr-sm-2 mb-sm-0">\
    ...
    <input id="user_endtime" type="text" class="form-control" value=""  placeholder="">\
  </div>\
  </div>\
<div class="form-group row">\
  <div class="input-group mb-2 mr-sm-2 mb-sm-0">\
    <div class="input-group-addon">用户数据</div>\
   <input id="user_user_data" type="text" class="form-control" value="'+ res[0].user_data +'"  placeholder="自定义数据">\
   ...
   var user_page = 1;
	function user_getpage (a_page,load) {
		...
		$.post("../User/lists",{
		    ...
		    menber_name:$("#search_user_menber_name").val(),
...
<!--用户日志-->
  <div class="tab-pane fade" id="user_log" role="tabpanel" aria-labelledby="user_log-tab">
  	
  	
  	<div class="py-2"></div>
  	<?php do_action('admin_user_log'); ?>
  	<div class="form-check form-check-inline">
  		<a href="javascript:;"  onclick="user_log_checkeddel();"  class="btn btn-danger">删除选中</a>
</div>
  ...
  <thead>
    <tr>
      <th><input onclick="$('input[name=user_log_checkbox]').prop('checked',$(this).prop('checked'));" type="checkbox"></th>
      <th>编号</th>
      <th>用户名</th>
      <th>归属程序</th>
      <th>类型</th>
  ...
  <tbody id="user_log_table">
 
  </tbody>
</table>
  	
<nav aria-label="Page  navigation example" >
  <ul class="pagination justify-content-end" id="user_log_page">
...
<script type="text/javascript">
function user_log_checkeddel () {
    	//遍历多选
    	var id_array=new Array();  
    	$('input[name=user_log_checkbox]:checked').each(function(){  
        id_array.push($(this).val());//向数组中添加元素  
        });
        var ids=id_array.join(',');//将数组元素连接起来以构建一个字符串  
        //遍历结束
        ...
        layer.confirm('你将无法恢复！', {
			...
			bx_load("正在删除...");
			$.post("../UserLog/delete",{id:ids},
    			function(result){
    			bx_load_close();
    			bx_msg(result,'user_log_getpage("",true)');
    		});
		});   
...
$("#user_log-tab").one("click",function(){user_log_getpage(1);});
	       
	       
	       
	//语言转换
   var user_log_lang = Array();
   user_log_lang['user_log_type'] = Array();
   user_log_lang['user_log_type']['1'] = '注册';
   user_log_lang['user_log_type']['2'] = '充值';
   user_log_lang['user_log_type']['3'] = '改密';
   user_log_lang['user_log_type']['4'] = '登陆';
   user_log_lang['user_log_type']['5'] = '退出';
   user_log_lang['user_log_type']['6'] = '解绑';
   user_log_lang['user_log_type']['7'] = '试用';
   user_log_lang['user_log_type']['8'] = '心跳';
 ...
 */
   //定义全局 储存页个数
   var user_log_limit = <?php echo $config['table_num'] ?>;
   var user_log_page = 1;
	function user_log_getpage (a_page,load) {
		if(load == null){
		loading_start();
		}
		if(a_page == ""){
    		a_page = user_log_page;
    	}else{
    		user_log_page = a_page;
    	}
		$.post("../UserLog/lists",{
			page:a_page,
			limit:user_log_limit,
			},
    		function(data){
    			//处理空数据
    			if(data == ""){
    				...
    				$("#user_log_page").html("");
    					...
    					user_log_getpage(user_log_page-1);
    				}
    				return;
    			}
    			res=JSON.parse(data);
    			...
    			for (var i = 0; i < res.data.length; i++) {
    				body +="<tr>\
    				<th scope=\"row\"><input name=\"user_log_checkbox\" type=\"checkbox\" value=\"" + res.data[i]["id"] + "\" ></th>\
    				<th>" + res.data[i]["id"] + "</th>\
    				<td>" + res.data[i]["username"] +  "</td>\
    				<td>" + res.data[i]["software_name"] +  "</td>\
    				<td>"+user_log_lang['user_log_type'][res.data[i]["type"]]+"</td>\
    				<td>"+res.data[i]["details"]+"</td>\
    				<td>"+res.data[i]["ip"]+"</td>\
    				<td>"+bx_time(res.data[i]["time"])+"</td>";
                }
    			$("#user_log_table").html(body);
    			$("#user_log_page").html(bx_page(res.count,a_page,user_log_limit,'user_log_getpage'));
    			if(load == null){
    			loading_close();
    			}
    		});
